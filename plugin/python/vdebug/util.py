...
def autocmd(pydbgp=None):
    ...
    if pydbgp:
        _autocmd = 'python -S ' + pydbgp + ' -d localhost:9000 ' + fname 
        print autocmd
        vim.command( "let cmd=system('%s')" % _autocmd )
        # vim.os.system(autocmd)

        # autocmd = 'python -S C:\Python27\Scripts\pydbgp.py -d localhost:9000 ' + fname 
